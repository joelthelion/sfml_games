...
public:
  TextLoader()
      : colors({Color::Black, Color::White, Color::Blue, Color::Red,
                ...
                Color(50, 128, 70), Color(130, 60, 30)}) {
  ...
  std::vector<sf::Text> getRandomTexts() {
    ...
    for (unsigned int i = 0; i < choice.size(); ++i) {
      ...
      output[i].setFillColor(
          colors[(std::find(letters.begin(), letters.end(), choice[i]) -
                  letters.begin()) %
                 colors.size()]);
    }
    return output;
...
restart:
  for (auto it = sprites.begin(); it != sprites.end(); ++it) {
    it->setOrigin(0, 0);
    it->setPosition(rand() % (int)(width - it->getGlobalBounds().width -
                                   it->getLocalBounds().left),
                    rand() % (int)(height - it->getLocalBounds().height -
                                   it->getLocalBounds().top));
    for (auto other = sprites.begin(); other != it; ++other) {
      if (tries < 1e5 &&
          it->getGlobalBounds().intersects(other->getGlobalBounds())) {
        tries++;
        goto restart;
      }
    }
